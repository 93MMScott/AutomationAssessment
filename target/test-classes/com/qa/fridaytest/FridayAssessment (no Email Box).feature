...
Scenario Outline: Add a User to the database
  Given that you are on the create UserScreen
  When the User details "<Username>" username, "<Password>" password, "<ConfirmPassword>" confirm Password, "<Email>" Email and "<FullName>" Fullname are entered on the Create UserScreen 
  And the details are submitted on the Create UserScreen
  Then the "<Username>" username should be visible on the UsersScreen
  
Examples:
  | Username | Password | ConfirmPassword | Email | FullName |
  | chris95 | guest | guest | chris@email.com | Christopher Perrins |
  | m4tt | chickenWing | chickenWing | chickenwing@email.com | matthewhunt |
  | dev | data | data | dev@email.com | Dev Gonsai | 
  | matthewhunt | chickenWing | chickenWing | matt@email.com | matttt |
